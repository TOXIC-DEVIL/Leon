module.exports = {
  ...
  func: async (sock, msg, text) => {
    ...
    if (msg.replied.image) {
      ...
      return await msg.reply({ image: image, caption: msg.replied.image?.caption || '' });
    } else if (msg.replied.video) {
      let video = await msg.load(msg.replied.video);
      return await msg.reply({ video: video, caption: msg.replied.video?.caption || '' });
    } else if (msg.replied.audio) {
      let audio = await msg.load(msg.replied.audio);
      return await msg.reply({ audio: audio, ptt: msg.replied.audio?.ptt || false });
    } else {
